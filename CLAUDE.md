...
- Entity pages: `artist.html`, `venue.html` (name, city, a small map, favorite toggle, upcoming + past), `promoter.html` (upcoming + past, "often books" artists, FOLLOW placeholder). Venue and promoter names across the app (list rows, detail panel, artist/venue/promoter gig rows) link to these pages when RA gives an id. List rows are a focusable `div[role=button]` (not a `<button>`) so those inner links are valid nested interactives — the row opens the panel except when a link is tapped.
